...
function core_upgrade_preamble() {
	...
	if ( $updates && ( count( $updates ) > 1 || 'latest' !== $updates[0]->response ) ) {
		...
		echo '<p>' . sprintf(
			...
			esc_url( self_admin_url( 'template-about.php' ) ),
			$normalized_version
		) . '</p>';
	}
...
function do_core_upgrade( $reinstall = false ) {
	...
	show_message(
		'<span class="hide-if-no-js">' . sprintf(
			...
			$result,
			esc_url( self_admin_url( 'template-about.php?updated' ) )
		) . '</span>'
	);
	show_message(
	...
	<script type="text/javascript">
	window.location = '<?php echo esc_url( self_admin_url( 'template-about.php?updated' ) ); ?>';
	</script>
	<?php
}
